...
const POSSIBLE_PATH =
  (process.platform === 'win32') ? [
    path.join(app.getPath('documents'), 'My Games\\Path of Exile 2\\production_Config.ini')
  ] : (process.platform === 'linux') ? [
    path.join(app.getPath('documents'), 'My Games/Path of Exile 2/production_Config.ini'),
    path.join(app.getPath('home'), '.local/share/Steam/steamapps/compatdata/238960/pfx/drive_c/users/steamuser/Documents/My Games/Path of Exile 2/production_Config.ini')
  ] : (process.platform === 'darwin') ? [
    path.join(app.getPath('appData'), 'Path of Exile 2/Preferences/production_Config.ini')
  ] : []
